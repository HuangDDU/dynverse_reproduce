...
run_fun <- function(expression, parameters, priors, verbose, seed) {
  ...
  output <- dynwrap::add_trajectory(output, milestone_network = cluster_network, progressions = progressions) # 这里看进入看看函数的具体执行
  output <- dynwrap::add_dimred(output, dimred = dimred)
  output <- dynwrap::add_timings(output, checkpoints)

  # 这里保存一堆散件, 方便调用Python的数据转化和add_trajectory调用的学习
  # 但这里不是dataset不好保存
  # output_dir <- "tmp/output"
  # dyncli::write_output(rownames(expression), paste(output_dir, "cell_ids.h5", sep='/'), "cell_ids")
  # dyncli::write_output(cluster_network, "tmp/output/cluster_network.h5", "cluster_network")
  # dyncli::write_output(progressions, "tmp/output/progressions.h5", "progressions")
  # dyncli::write_output(dimred, "tmp/output/dimred.h5", "dimred")
  # dyncli::write_output(checkpoints, "tmp/output/checkpoints.h5", "checkpoints")
  # 散件不好保存, 那就保存到output里, 额外添加一个键值, 后续会保存
  l <- list()
  l$cell_ids <- rownames(expression)
  l$cluster_network <- cluster_network
  l$progressions <- progressions
  l$dimred <- dimred
