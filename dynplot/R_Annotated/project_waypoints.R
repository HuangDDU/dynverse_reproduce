...
project_waypoints_coloured <- function(
  ...
  cell_positions,
  edge_positions = NULL,
  waypoints = dynwrap::select_waypoints(trajectory), # 这里在Python中在函数体内部具体中调用
  ...
  waypoint_positions <-
    if (!is.null(edge_positions)) {
      ...
      # 使用核和几何距离计算wps的低维降维
      weights <- wps$geodesic_distances %>% stats::dnorm(sd = trajectory_projection_sd) # 高斯核，距离越大权重越小
      assert_that(all(!is.na(weights)))

      weights <- weights / rowSums(weights) # 权重行归一化
  ...
  segments <- left_join(
    ...
    mutate(
      # 计算
      closest = which.min(abs(.data$percentage - 0.5)), # 找到最接近 0.5 的 percentage 的索引
