...
plot_dimred <- dynutils::inherit_default_params(
  ...
  function(
    ...
    # join链接操作降维结果与里程碑百分比调整细胞位置
    cell_positions <- left_join(
      cell_positions,
      # 选择每个细胞最接近的里程碑, 其实已经写有可以用的函数了gather_cells_at_milestones
      # gather_cells_at_milestones(trajectory)$milestone_percentages %>% select(.data$cell_id, .data$milestone_id)
    ...
    if (plot_trajectory) {
      ...
      # 投影waypoint
      # waypoint是结合里细胞与milestone network的采样点
      # 从milstone网络中节点和边的一堆采样点, 用来代替里程碑网络的节点,
      ...
      milestone_positions <- wp_segments %>% filter(!is.na(.data$milestone_id)) # 过滤只保留里程碑节点上的waypoint点, 就是milestone节点

      # 绘制waypoint构成的轨迹曲线
      # plot milestones
      # 里程碑绘制
      plot <- plot +
        geom_point( # waypoint关键milestone节点绘制,
          data = milestone_positions,
          color = "#333333",
          size = size_milestones #
        ) +
        geom_path( # 所有waypoint点绘制, 绘制轨迹曲线
